...
func MakeChannels(cfg *config.Config, sourceChain, destChain interfaces.IChain, logger *logrus.Logger) map[string]IChannel {
	...
	if cfg.Chain.Source.Enable {
		srcChannel, err := NewChannel(sourceChain, destChain, cfg.Chain.Source.Cache.StartHeight,cfg.Chain.Source.Cache.RevisedHeight, cfg.Chain.Source.Cache.Filename,cfg.Chain.Source.RelayFrequency, logger)
		if err != nil {
	...
	if cfg.Chain.Dest.Enable {
		destChannel, err := NewChannel(destChain, sourceChain, cfg.Chain.Dest.Cache.StartHeight,cfg.Chain.Dest.Cache.RevisedHeight, cfg.Chain.Dest.Cache.Filename,cfg.Chain.Dest.RelayFrequency, logger)
		if err != nil {
